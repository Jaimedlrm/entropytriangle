    ...
    if(all(hasCmetEntropicCoords(edf[l]) for l in range(len(edf)))):
        ...
        for i in range(3): 
            for j in range(len(edf)):
                ...
                if(i != 1):
                    tax.scatter(points, s = s_mk , marker = mk[j], label = names, color=colors(j), linewidth='0.1')
                
                else:
                    tax.scatter(points, s = s_mk , marker = mk[j], label = names, color ='white' , edgecolors = colors(j), linewidth='1.5')
